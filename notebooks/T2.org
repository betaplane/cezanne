...
#+begin_src ipython :results silent :session
  ...
  km = cluster.KMeans(n_clusters=2).fit_predict((err - err.mean('Time')).values)
  df = pd.concat((dz, sta['d03_var_sso'], pd.Series(km, index=err.station, name='km')), 1)
  sm = ScalarMappable(norm=Normalize(0, 9), cmap='tab10')
#+end_src
...
#+begin_src ipython :results raw :session
  e0 = err.sel(station = km==0)
  i = e0.argmax('Time')

  plt.figure()
  x = e0.sel(station = i==0)
  plt.plot(x.values.flatten(), label=x.station.item())
  x = e0.sel(station = i==23)
  plt.plot(x.values.flatten(), label=x.station.item())
  x = e0.sel(station = i==3)
  plt.plot(x.values.flatten(), label=x.station.item())
  plt.legend()
#+end_src

#+RESULTS:
[[file:./obipy-resources/T2/yQQxlG.png]]
#+begin_src ipython :results raw :session
  plt.figure()
  x = e0.sel(station = i==4)
  plt.plot(x.values.T)
  plt.legend(x.station.values)
#+end_src

#+RESULTS:
[[file:./obipy-resources/T2/IJzo4U.png]]
