...
#+begin_src ipython :results silent :session
  ...
  t0, t12 = separate_start_hours('/nfs/HPC/arno/data/T2_*_81*', 'T2')
  b0, b12 = [lead_day(0, T, d) for d in [t0, t12]]
#+end_src
...
#+begin_src ipython :results silent :session
  ...
  t1 = t12.sel(start=slice(None, '2017-07-18'))
  t2 = t12.sel(start=slice('2017-07-19', None))
  b1, b2 = [lead_day(0, T, d) for d in [t1, t2]]
#+end_src
...
#+begin_src ipython :results raw :session :savefig "mean_bias_d03_12h_day0.png"
  ...
  _, gl = coq.plotrow(x, subplot_spec=gs[0, 1:], vmin=-10, vmax=10, cbar_label='$^{\circ}$C', cbar_width=0.03)
  gl[0].ylabels_left = False
  cplots.rowlabel(gs[0, :], "pre", labelpad=80)
  x = pd.concat((b2.mean(), b2.mean() + 0.0065 * dz['dz']), 1)
  x.columns = ['dT', 'dTc']
  coq.plotrow(x, subplot_spec=gs[1, 1:], vmin=-10, vmax=10, cbar_label='$^{\circ}$C', cbar_width=0.03)
  cplots.rowlabel(gs[1, :], "post", labelpad=80)
#+end_src
...
#+begin_src ipython :results silent :session
  # where the change in GFS occurs
  b1, b2 = [lead_day(1, T, d) for d in [t1, t2]]
#+end_src
...
#+begin_src ipython :results silent :session
  T = hh.stationize(pd.read_hdf(hh.config.Field.file_name, 'ta_c').drop('10', 1, 'elev')) + 273.15
  g02 = xr.open_dataarray('/nfs/HPC/arno/data/T2_glacio_d02_intp.nc')
  g03 = xr.open_dataarray('/nfs/HPC/arno/data/T2_glacio_d03_intp.nc')
  b = xr.concat((g02, g03), 'domain')
  b.coords['Time'] = ('Time', b.XTIME)
  b = (b - xr.DataArray(T).rename({'ultima_lectura': 'Time'})).mean('Time')
  if b.dims.index('domain') == 0:
      b_glacio = pd.DataFrame(b.values, index=b.domain, columns=b.station).T
  else:
      b_glacio = pd.DataFrame(b.values, columns=b.domain, index=b.station)
  b_glacio.columns = ['d02', 'd03']
#+end_src
...
gl[0].ylabels_left = False
plt.set_cmap('Spectral_r')
_, gl = coq.plotrow(b_glacio, subplot_spec=gs[1:], vmin=-10, vmax=10, cbar_label='$^{\circ}$C', cbar_width=0.03)
gl[0].ylabels_left = False
#+end_src
...
#+begin_src ipython :results silent :session
  Tx = xr.DataArray(T).rename({'ultima_lectura': 'Time'}).mean('Time')
  x = pd.concat((
      b1.resample('QS-MAR').mean(),
      b2.resample('QS-MAR').mean(),
      xr2pd((time(g02) - Tx).resample('QS-MAR', 'Time')),
      xr2pd((time(g03) - Tx).resample('QS-MAR', 'Time'))
  ), keys=['pre', 'post', 'd02', 'd03'])
  idx = x.index.get_level_values(1)
  x.index = pd.MultiIndex.from_arrays((idx.month, idx.year, x.index.get_level_values(0)))
  x = x.sort_index().unstack(0)
#+end_src
...
#+begin_src ipython :results raw :session :savefig "bias_seasonal_2014.png"
  ...
  fig.subplots_adjust(wspace=0.04, hspace=0.04)
  gs = gridspec.GridSpec(2, 1)
  for i, (n, r) in enumerate(x.loc[2014].iterrows()):
      df = r.unstack()
      df.columns = [{3: 'MAM', 6: 'JJA', 9: 'SON', 12: 'DJF'}[k] for k in df.columns]
      coq.plotrow(df, subplot_spec=gs[i, 0], vmin=-10, vmax=10)
      cplots.rowlabel(gs[i, 0], n)
...
#+begin_src ipython :results raw :session :savefig "bias_seasonal_2015.png"
  ...
  fig.subplots_adjust(wspace=0.04, hspace=0.04)
  gs = gridspec.GridSpec(2, 1)
  for i, (n, r) in enumerate(x.loc[2015].iterrows()):
      df = r.unstack()
      df.columns = [{3: 'MAM', 6: 'JJA', 9: 'SON', 12: 'DJF'}[k] for k in df.columns]
      coq.plotrow(df, subplot_spec=gs[i, 0], vmin=-10, vmax=10)
      cplots.rowlabel(gs[i, 0], n)
...
#+begin_src ipython :results raw :session :savefig "bias_seasonal_2016.png"
  ...
  fig.subplots_adjust(wspace=0.04, hspace=0.04)
  gs = gridspec.GridSpec(3, 1)
  for i, (n, r) in enumerate(x.loc[2016].iterrows()):
      df = r.unstack()
      df.columns = [{3: 'MAM', 6: 'JJA', 9: 'SON', 12: 'DJF'}[k] for k in df.columns]
      coq.plotrow(df, subplot_spec=gs[i, 0], vmin=-10, vmax=10)
      cplots.rowlabel(gs[i, 0], n)
...
#+begin_src ipython :results raw :session :savefig "bias_seasonal_2017.png"
  fig = plt.figure(figsize=(8, 7))
  fig.subplots_adjust(wspace=0.04, hspace=0.04)
  gs = gridspec.GridSpec(2, 1)
  for i, (n, r) in enumerate(x.loc[2017].sort_index(ascending=False).iterrows()):
      df = r.unstack()
      df.columns = [{3: 'MAM', 6: 'JJA', 9: 'SON', 12: 'DJF'}[k] for k in df.columns]
      coq.plotrow(df, subplot_spec=gs[i, 0], vmin=-10, vmax=10)
      cplots.rowlabel(gs[i, 0], n)
...
#+RESULTS:
[[file:./obipy-resources/T2/bias_seasonal_2017.png]]

#+begin_src ipython :results raw :session :savefig "bias_seasonal_2018.png"
  fig = plt.figure(figsize=(8, 4))
  fig.subplots_adjust(wspace=0.04, hspace=0.04)
  gs = gridspec.GridSpec(1, 1)
  df = x.loc[2018].stack().T
  df.columns = [{3: 'MAM', 6: 'JJA', 9: 'SON', 12: 'DJF'}[k] for k in df.columns.droplevel(0)]
  coq.plotrow(df, subplot_spec=gs[0], vmin=-10, vmax=10)
  cplots.rowlabel(gs[0], 'post')
#+end_src

#+RESULTS:
[[file:./obipy-resources/T2/bias_seasonal_2018.png]]

#+begin_src ipython :results silent :session
from filters import Lanczos
f0 = Lanczos.filter(b0, '30D')
f0 = Lanczos.filter(b0, '30D')
#+end_src
