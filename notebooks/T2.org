...
#+begin_src ipython :results silent :session
  ti = xr.open_dataarray('/nfs/HPC/arno/data/T2_glacio_intp.nc')
  b_glacio = (pd.DataFrame(ti, index=ti.XTIME, columns=ti.station) - T).dropna(1, 'all').dropna(0, 'all')
#+end_src


#+begin_src ipython :results raw :session
fig = plt.figure()
coq.plotrow(b_glacio.mean().to_frame('dT'))
#+end_src

#+RESULTS:
[[file:./obipy-resources/T2/RORFPf.png]]
