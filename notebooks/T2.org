...
#+begin_src ipython :results raw :session :savefig "SMAP_SMOIS_avg.png"
  vmax = .5
  fig, axs = plt.subplots(1, 3, figsize=(9, 5), subplot_kw={'projection': crs.PlateCarree()})
  fig.subplots_adjust(hspace=0.04)
  plt.set_cmap('cividis')
  x = smap.mean('time')
  lon = (lambda k, l: k - np.r_[l[0], l] / 2)(x.lon, np.diff(x.lon))
  lat = (lambda k, l: k - np.r_[l[0], l] / 2)(x.lat, np.diff(x.lat))
  pl = axs[0].pcolormesh(lon, lat, x, transform=crs.PlateCarree(), vmin=0, vmax=vmax)
  axs[0].scatter(*hh.sta.loc[['CNFJB', 'CNFJQ'], ['lon', 'lat']].astype(float).as_matrix().T, marker='o', color='r', transform=crs.PlateCarree())
  coq(axs[0], lines_only=True, colors=['w'])
  axs[0].background_patch.set_color('none')
  axs[0].set_extent(coq.bbox)
  axs[0].outline_patch.set_edgecolor('w')
  axs[0].set_title('SMAP')

  y = layer(0)
  pl = axs[1].pcolormesh(y.XLONG[0,:,:], y.XLAT[0,:,:], y.mean(('start', 'Time')) * lm, transform=crs.PlateCarree(), vmin=0, vmax=vmax)
  coq(axs[1], lines_only=True, colors=['w'])
  axs[1].scatter(*hh.sta.loc[['CNFJB', 'CNFJQ'], ['lon', 'lat']].astype(float).as_matrix().T, marker='o', color='r', transform=crs.PlateCarree())
  axs[1].background_patch.set_color('none')
  axs[1].outline_patch.set_edgecolor('w')
  axs[1].set_extent(coq.bbox)
  axs[1].set_title('SMOIS layer 1')

  y = layer(1)
  pl = axs[2].pcolormesh(y.XLONG[0,:,:], y.XLAT[0,:,:], y.mean(('start', 'Time')) * lm, transform=crs.PlateCarree(), vmin=0, vmax=vmax)
  coq(axs[2], lines_only=True, colors=['w'])
  axs[2].scatter(*hh.sta.loc[['CNFJB', 'CNFJQ'], ['lon', 'lat']].astype(float).as_matrix().T, marker='o', color='r', transform=crs.PlateCarree())
  axs[2].background_patch.set_color('none')
  axs[2].outline_patch.set_edgecolor('w')
  axs[2].set_extent(coq.bbox)
  axs[2].set_title('SMOIS layer 2')
  cplots.cbar(pl, 'right', axs[2], width=.02, space=.02)
#+end_src

#+CAPTION: Average soil moiusture (as fraction) from SMAP and the NOAH top two model layers. The time window for the average is from 2015-08 (start SMAP) to 2017-07 (see above). Red dots indicate stations Fray Jorge Bosque (CNFJB) and Fray Jorge Quebrada (CNFJQ), for which soil moisture data is available.
#+RESULTS:
[[file:./obipy-resources/T2/SMAP_SMOIS_avg.png]]
...
#+begin_src ipython :results silent :session
  ...
  i = np.vectorize(lambda x: abs(smap.lon - x).argmin().item())(lon)
  j = np.vectorize(lambda x: abs(smap.lat - x).argmin().item())(lat)
...
|--------------------+-------+--------------------------------------|
| sf_surface_physics |     1 | thermal diffusion only (no moisture) |
