...
|       |               -0.1 |               -0.2 |               -0.5 |                SMAP |
|-------+--------------------+--------------------+--------------------+---------------------|
| CNFJB | 16.227916633775212 | 17.436791901087354 | 16.321969650409876 | 0.14533954858779907 |
| CNFJQ | 14.575972260904477 | 15.108363282918049 |  15.14085535884631 | (same grid box)     |

#+begin_src ipython :results raw :session
from geo import nearest
i, j, _ = nearest(*np.meshgrid(smap.lon, smap.lat), *sta.loc['CNFJB', ['lon', 'lat']])
smap.isel(lat=i,lon=j).mean('time').item()
#+end_src
...
#+begin_src ipython :results silent :session
  ...
  lm[lm==0] = np.nan
  smap = xr.open_dataarray('/nfs/HPC/arno/data/SMAP_SPL3SMP.nc').sel(time=slice(None, '2017-07-18'))
...
#+begin_src ipython :results raw :session :savefig "SMAP_SMOIS_avg_var.png"
  ...
  x = smap.mean('time')
  lon = (lambda k, l: k - np.r_[l[0], l] / 2)(x.lon, np.diff(x.lon))
  lat = (lambda k, l: k - np.r_[l[0], l] / 2)(x.lat, np.diff(x.lat))
...
***** based simply on minimum distance to SMAP gridpoint (center)
 #+begin_src ipython :results silent :session
   ...
   i = np.vectorize(lambda x: abs(smap.lat - x).argmin().item())(lat)
   j = np.vectorize(lambda x: abs(smap.lon - x).argmin().item())(lon)
...
***** to confirm that the simple nearest-neighbor calculation above approximately works
  ...
  #+end_src
  #+RESULTS:
  [[file:./obipy-resources/T2/2QNlQK.png]]
...
**** results
 #+begin_src ipython :results silent :session
   ...
   smap_c = smap.copy()
   smap_c['b'] = (('lat', 'lon'), -np.ones(smap.shape[1:]))
   for k in np.unique(i):
       for l in np.unique(j):
           try:
               ...
               pass
   smap_st = smap_c.stack(bin = ('lat', 'lon'))
...
#+RESULTS:
[[file:./obipy-resources/T2/fGt0m1.png]]
