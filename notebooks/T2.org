...
D = pd.HDFStore('../../ceaza/data/stations.h5')
sta = D['stations']
#+end_src
...
#+begin_src ipython :results raw :session :savefig "landmask.png"
  ...
  for i, (n, c) in enumerate(d.iteritems()):
      axs[i].scatter(*ll, c=c, transform=crs.PlateCarree())
      coq(axs[i])
      axs[i].set_title(n)
      ...
      if i < d.shape[1] - 1: gl.ylabels_right = False
#+end_src
...
** TODO move stations.h5 path onto CEAZAMet object, with traitlets
