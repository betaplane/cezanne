...
x = pd.concat((dz, sta['d03_var_sso']), 1).dropna()
x = x[abs(x['dz'])<100]
idx = T.columns.intersection(x.index)
n = (datetime.now() - datetime(2015, 8, 15)).days * 24
a = T.loc['2015-08-15-:, idx].count() / n
#+end_src
...
coq.plotrow(x.loc[idx, 'd03_var_sso'].to_frame() ** .5, gs[0], cbar_kw={'space': .002})
coq.plotrow(a.to_frame('avail'), gs[1], ylabels=False, cbar_kw={'space': .002})
#+end_src

#+CAPTION: CEAZAMet stations where the model grid (domain d03) is within +/- 100m of the true station elevation. **Right** - square root of subgridscale orographic varianca ('VAR_SSO' in 'geo_em...nc' file); **left** - data availbility as a fraction of the time since 2015-08-15 (beginning of WRF operacional data).

#+RESULTS:
[[file:./obipy-resources/T2/bRF9HK.png]]
...
#+begin_src ipython :results silent :session
  ...
  T = hh.stationize(pd.read_hdf(hh.config.Field.file_name, 'ta_c').drop('10', 1, level='elev')) + 273.15
  dz = (hh.sta['d03'] - hh.sta['elev'].astype(float)).to_frame('dz')
...
#+RESULTS:
[[file:./obipy-resources/T2/MAE_min_max_corr.png]]
...
#+begin_src ipython :results raw :session :savefig "HS_FrayJorge.png"
hs.plot(figsize=(8, 3))
#+end_src

#+RESULTS:
[[file:./obipy-resources/T2/HS_FrayJorge.png]]
...
#+begin_src ipython :results raw :session
tab = hs[:'2012-10'].mean().unstack() # sensor failure, see figure above
...
|       |      -0.1 |      -0.2 |      -0.5 | SMAP            |
|-------+-----------+-----------+-----------+-----------------|
| CNFJB | 45.765119 | 41.971980 | 45.970497 | 0.144           |
| CNFJQ | 42.292605 | 43.845763 | 43.985203 | (same grid box) |

#+begin_src ipython :results raw :session
from geo import nearest
i, j, _ = nearest(*np.meshgrid(smap.col, smap.row), *hh.sta.loc['CNFJB', ['lon', 'lat']])
smap.soil_moisture.isel(row=i,col=j).mean('time').item()
#+end_src

#+RESULTS:
: 0.14400552213191986

*** mean fields (SMAP vs. NOAH)
#+begin_src ipython :results silent :session
  lm = xr.open_dataset('/home/arno/Documents/data/WRF/3d/geo_em.d03.nc')['LANDMASK'].squeeze()
  lm[lm==0] = np.nan
  smap = xr.open_dataset('/nfs/HPC/arno/data/SMAP_SPL3SMP.nc').sel(time=slice(None, '2017-07-18'))
  smois_121 = xr.open_dataarray('/nfs/HPC/arno/data/SMOIS_field_121.nc')
  def layer(i):
      smois_145 = xr.open_dataarray('/nfs/HPC/arno/data/SMOIS_field_145_layer{}.nc'.format(i+1))
      return xr.concat((
          smois_121.isel(Time=slice(24, 48), soil_layers_stag=0),
          smois_145.isel(Time=slice(24, 48)
      )), 'start').sel(start=slice('2015-08-15', '2017-07-18'))
#+end_src

#+begin_src ipython :results raw :session :savefig "SMAP_SMOIS_avg_var.png"
  def figbg(ax, title=None):
      coq(ax, lines_only=True, colors=['w'])
      ax.background_patch.set_color('none')
      ax.set_extent(coq.bbox)
      ax.outline_patch.set_edgecolor('w')
  ...
  plt.set_cmap('cividis')
  gs = gridspec.GridSpec(2, 3, hspace=.04, wspace=.02)

  ax = fig.add_subplot(gs[0, 0], projection=crs.PlateCarree())
  x = smap.soil_moisture.mean('time')
  lon = (lambda k, l: k - np.r_[l[0], l] / 2)(x.col, np.diff(x.col))
  lat = (lambda k, l: k - np.r_[l[0], l] / 2)(x.row, np.diff(x.row))
  pl = ax.pcolormesh(lon, lat, x, transform=crs.PlateCarree(), vmin=0, vmax=vmax)
  ax.scatter(*hh.sta.loc[['CNFJB', 'CNFJQ'], ['lon', 'lat']].astype(float).as_matrix().T, marker='o', color='r', transform=crs.PlateCarree())
  figbg(ax, 'SMAP')

  ax = fig.add_subplot(gs[1, 0], projection=crs.PlateCarree())
  x = smap.soil_moisture.std('time')
  pl = ax.pcolormesh(lon, lat, x, transform=crs.PlateCarree(), vmin=0, vmax=vstd)
  ax.scatter(*hh.sta.loc[['CNFJB', 'CNFJQ'], ['lon', 'lat']].astype(float).as_matrix().T, marker='o', color='r', transform=crs.PlateCarree())
  figbg(ax, None)

  ax = fig.add_subplot(gs[0, 1], projection=crs.PlateCarree())
  y = layer(0)
  pl = ax.pcolormesh(y.XLONG[0,:,:], y.XLAT[0,:,:], y.mean(('start', 'Time')) * lm, transform=crs.PlateCarree(), vmin=0, vmax=vmax)
  coq(ax, lines_only=True, colors=['w'])
  ax.scatter(*hh.sta.loc[['CNFJB', 'CNFJQ'], ['lon', 'lat']].astype(float).as_matrix().T, marker='o', color='r', transform=crs.PlateCarree())
  figbg(ax, 'SMOIS layer 1')

  ax = fig.add_subplot(gs[1, 1], projection=crs.PlateCarree())
  pl = ax.pcolormesh(y.XLONG[0,:,:], y.XLAT[0,:,:], y.mean('Time').std('start') * lm, transform=crs.PlateCarree(), vmin=0, vmax=vstd)
  coq(ax, lines_only=True, colors=['w'])
  ax.scatter(*hh.sta.loc[['CNFJB', 'CNFJQ'], ['lon', 'lat']].astype(float).as_matrix().T, marker='o', color='r', transform=crs.PlateCarree())
  figbg(ax, None)
  ...
  coq(ax, lines_only=True, colors=['w'])
  ax.scatter(*hh.sta.loc[['CNFJB', 'CNFJQ'], ['lon', 'lat']].astype(float).as_matrix().T, marker='o', color='r', transform=crs.PlateCarree())
  figbg(ax, 'SMOIS layer 2')
  cplots.cbar(pl, 'right', ax, width=.02, space=.02)

  ax = fig.add_subplot(gs[1, 2], projection=crs.PlateCarree())
  pl = ax.pcolormesh(y.XLONG[0,:,:], y.XLAT[0,:,:], y.mean('Time').std('start') * lm, transform=crs.PlateCarree(), vmin=0, vmax=vstd)
  coq(ax, lines_only=True, colors=['w'])
  ax.scatter(*hh.sta.loc[['CNFJB', 'CNFJQ'], ['lon', 'lat']].astype(float).as_matrix().T, marker='o', color='r', transform=crs.PlateCarree())
  figbg(ax, None)
  ...
  cplots.row_label(gs[1, :], 'std')
  cplots.row_label(gs[0, :], 'avg')
#+end_src

#+CAPTION: Top row: Average soil moiusture (as fraction) from SMAP and the NOAH top two model layers. Bottom row: StDev - in the case of SMOIS in WRF, after first averaging daily, so as to facilitate comparison to SMAP. *NOTE*: The daily variability is higher in the second soil layer in NOAH. The time window for the average is from 2015-08 (start SMAP) to 2017-07 (see above). Red dots indicate stations Fray Jorge Bosque (CNFJB) and Fray Jorge Quebrada (CNFJQ), for which soil moisture data is available.
#+RESULTS:
[[file:./obipy-resources/T2/SMAP_SMOIS_avg_var.png]]


*** domain average (and comparison to T2) over time
**** a simple calculation which WRF gridpoints fall into which SMAP grid cells
***** based simply on minimum distance to SMAP gridpoint (center)
 #+begin_src ipython :results silent :session
   lon, lat = smois_121.XLONG[0, :, :], smois_121.XLAT[0, :, :]
   ...
   s = list(set(z))
   ij = np.array([s.index(e) for e in z]).reshape(i.shape)
 #+end_src

***** to confirm that the simple nearest-neighbor calculation above approximately works
  #+begin_src ipython :results raw :session
    ax = plt.axes(projection=crs.PlateCarree())
    ax.pcolormesh(lon, lat, ij, transform=crs.PlateCarree())
    coq(ax, lines_only=True)
  #+end_src
  #+RESULTS:
  [[file:./obipy-resources/T2/vAnUln.png]]

**** results
 #+begin_src ipython :results silent :session
   y = (layer(0) * lm).mean('Time')
   y['bin'] = (('south_north', 'west_east'), ij)
   smois_smap = y.groupby('bin').mean(axis=-1)
   smois_smap['start'] = ('start', pd.DatetimeIndex(smois_smap.indexes['start'].date))
   smois_smap = smois_smap.rename({'start': 'time'})

   smap_c = smap.soil_moisture.copy()
   smap_c['b'] = (('row', 'col'), -np.ones(smap_c.shape[1:]))
   for k in np.unique(i):
       for l in np.unique(j):
           try:
               smap_c.b[k, l] = s.index((k, l))
           except ValueError:
               pass
   smap_st = smap_c.stack(bin = ('row', 'col'))
   smap_st = smap_st.sel(bin = smap_st.b>=0)
   smap_st['bin'] = smap_st.b
   smap_st = smap_st.drop('b')
 #+end_src

#+begin_src ipython :results silent :session
  from geo import stations_in_domain
  sta = stations_in_domain(hh.sta, '/home/arno/Documents/data/WRF/3d/geo_em.d03.nc')

  k = hh.data_gaps(T.loc['2015-05':'2017-07'], 48).intersection(sta.index)
  ...
  def DF(x):
      y = x.isel(Time=slice(24, 48)).stack(t=('Time', 'start')).sortby('XTIME')
      return pd.DataFrame(y.transpose('t', 'station').values, index=y.XTIME, columns=y.station)

  dtr_wrf = pd.concat((DTR(DF(T2_0h.sel(station=sta.index))), DTR(DF(T2_12h_preJul17.sel(station=sta.index)))))
  dtr_T = DTR(T[sta.index.intersection(T.columns)])
#+end_src

 #+begin_src ipython :results raw :session :savefig "SMOIS_T2.png"
   colors = plt.rcParams['axes.prop_cycle'].by_key()['color']

   fig = plt.figure(figsize=(10, 6))
   smap_st.mean('bin').dropna('time').plot(color=colors[0], label='SMAP')
   smois_smap.mean('bin').plot(color=colors[1], label='SMOIS')
   abs(smois_smap - smap_st).mean('bin').plot(color=colors[2], label='MAE')
   x = pd.concat((bias_0h.abs().mean(1).resample('D').mean(), bias_12h_preJul17.abs().mean(1).resample('D').mean()))
   plt.legend()
   bx = fig.axes[0].twinx()
   (dtr_wrf - dtr_T).abs().mean(1).plot(color=colors[4], label='DTR MAE')
   x.plot(color=colors[3], label='T2 MAE')
   plt.legend()
 #+end_src

 #+CAPTION: Soil moisture and T2 errors: 'SMOIS' refers to the top layer of the NOAH LSM, averaged over the model domain and daily, 'SMAP' to the SMAP values (also averaged over the grid cells corresponding to our WRF domain, and 'MAE' to the absolute error. For 'MAE', the WRF variable ('SMOIS') is averaged across blocks of grid cells approximately corresponding to the grid cells of the SMAP product (~ 36 x 36 km) and daily, and then the absolute difference is computed and averaged over the domain. 'DTR' stands for daily temperature range.
 #+RESULTS:
 [[file:./obipy-resources/T2/SMOIS_T2.png]]


#+begin_src ipython :results raw :session :savefig "DTR.png"
  colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
  R = pd.read_hdf('/home/arno/Documents/data/CEAZAMet/station_data.h5', 'pp_mm').xs('prom', 1, level='aggr').resample('D').sum()

  fig = plt.figure(figsize=(10, 6))
  plt.subplot2grid((3, 1), (0, 0), rowspan=2)
  dtr_T.mean(1).plot(label='stations')
  dtr_T[k].mean(1).plot(label='stations restr')
  dtr_wrf.mean(1).plot(label='WRF')
  dtr_wrf[k].mean(1).plot(label='WRF restr')
  plt.gca().set_ylim(0, 20)
  plt.gca().set_ylabel('$^{\circ}$C')
  plt.legend(loc=2)
  bx = plt.gca().twinx()
  abs(dtr_wrf - dtr_T).mean(1).plot(label='diff', color=colors[4])
  abs(dtr_wrf[k] - dtr_T[k]).mean(1).plot(label='diff restr', color=colors[5])
  plt.legend(loc=1)
  bx.set_ylabel('$^{\circ}$C')

  plt.subplot2grid((3, 1), (2, 0), sharex=bx)
  r = R.columns.get_level_values('station').intersection(sta.index)
  R[r.intersection(k)].mean(1).plot(label='restr')
  R[r].mean(1).plot(label='all')
  plt.legend()
  plt.gca().set_xlim('2015-05', '2017-08')
  plt.gca().set_ylabel('mm day$^{-1}$')
#+end_src

#+CAPTION: Mean (for several stations) Daily Temperature Ranges (DTR) of station data and WRF simulations interpolated to station locations. The series with 'restr' in the labels restrict the mean to stations who do not have gaps longer than 48h between May 2015 and July 2017. The right hand scale of the top plot (series labels with 'diff') represent the mean absolute difference between the DTRs from stations and WRF. The lower panel plots station precipitation along the same time axis, with means formed in the same way as for the DTRs.
#+RESULTS:
[[file:./obipy-resources/T2/DTR.png]]

*** tests
**** 2016/08 - 2016/10 (or just 2016/10)
**** soil moisture
| namelist           | value | meaning                              | notes / questions                                 |
|--------------------+-------+--------------------------------------+---------------------------------------------------|
| sf_surface_physics |     0 | no sfc temp prediction               | does that mean no T2 etc? (-> v. short run first) |
|                    |     1 | thermal diffusion only (no moisture) |                                                   |
|                    |     7 | Pleim-Xiu LSM (with nudging)         |                                                   |
***** also, continuous soil property simulation across runs
****** [[zotero://select/items/1_IX6UY7A8][Xiu, Aijun, and Jonathan E. Pleim. “Development of a Land Surface Model. Part I: Application in a Mesoscale Meteorological Model.” Journal of Applied Meteorology 40, no. 2 (February 2001): 192–209. https://doi.org/10.1175/1520-0450(2001)040<0192:DOALSM>2.0.CO;2.]]
***** one test where soil moisture in met_em... is dramatically altered **during** the course of a simulation - to see if only initial data are used by NOAH or also continuous BCs.
**** initialization
***** DFI (digital filter initialization)
  | namelist - &dfi_control  |  value | meaning                               |
  |--------------------------+--------+---------------------------------------|
  | dfi_opt                  |      0 | off (also needed for restart run)     |
  |                          |      1 | DFL (digital filter launch)           |
  |                          |      2 | DDFI (diabatic DFI)                   |
  |                          |      3 | TDFI (twice DFI, recommended)         |
  | dfi_nfilter              |      0 | uniform                               |
  |                          |      1 | Lanczos                               |
  |                          |      2 | Hamming                               |
  |                          |      3 | Blackman                              |
  |                          |      4 | Kaiser                                |
  |                          |      5 | Potter                                |
  |                          |      6 | Dolph window                          |
  |                          |      7 | Dolph (default, recommended)          |
  |                          |      8 | recursive high-order filter           |
  | dfi_write_filtered_input | .true. | write out filtered state before start |
  | ... more options         |        |                                       |
  | constant_bc              |        | boundary conditions in &bdy_control   |
  | time_step_dfi            |        |                                       |

****** track error across forecast lead time
***** grid / obs nudging - &fdda
***** obsgrid (same as nudging?)
****** Pleim-Xiu LSM nudging
**** topo_shading = 1, slope_rad = 1 (depends on ra_sw_physics?)
**** SST
**** obsgrid observation nudging
*** tasks
**** TODO air humidity in model and data
***** including radiosondes
**** TODO soil moisture evolution over longer simulation (e.g. WRF_GLACIO)
**** TODO look in more detail at humedad del suelo timeseries
***** does it even change, connection to ppt
** Skewness & kurtosis
