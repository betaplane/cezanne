...
#+begin_src ipython :results raw :session :savefig "SMAP_SMOIS_avg_var.png"
  ...
  pl = ax.pcolormesh(y.XLONG[0,:,:], y.XLAT[0,:,:], y.std(('start', 'Time')) * lm, transform=crs.PlateCarree(), vmin=0, vmax=vstd)
  coq(ax, lines_only=True, colors=['w'])
  ax.scatter(*hh.sta.loc[['CNFJB', 'CNFJQ'], ['lon', 'lat']].astype(float).as_matrix().T, marker='o', color='r', transform=crs.PlateCarree())
  figbg(ax, None)
  ...
  pl = ax.pcolormesh(y.XLONG[0,:,:], y.XLAT[0,:,:], y.mean(('start', 'Time')) * lm, transform=crs.PlateCarree(), vmin=0, vmax=vstd)
  coq(ax, lines_only=True, colors=['w'])
  ax.scatter(*hh.sta.loc[['CNFJB', 'CNFJQ'], ['lon', 'lat']].astype(float).as_matrix().T, marker='o', color='r', transform=crs.PlateCarree())
  figbg(ax, None)
  cplots.cbar(pl, 'right', ax, width=.02, space=.02)
...
#+begin_src ipython :results silent :session
  ...
  dtr_wrf = pd.concat((DTR(bias_0h), DTR(bias_12h_preJul17))).mean(1)
  dtr_T = DTR(T).mean(1)
#+end_src
 ...
 #+begin_src ipython :results raw :session :savefig "SMOIS_T2.png"
   ...
   plt.legend()
   bx = fig.axes[0].twinx()
   (dtr_wrf - dtr_T).abs().plot(color=colors[4], label='DTR MAE')
...
fig = plt.figure(figsize=(10, 6))
dtr_wrf.plot(label='WRF')
dtr_T.plot(label='stations')
plt.gca().set_xlim('2015-05', '2017-08')
plt.gca().set_ylim(0, 20)
plt.legend()
#+end_src

#+CAPTION: Daily Temperature Ranges (DTR) for the mean over all station locations, for WRF results (interpolated to station locations) and the station data.
#+RESULTS:
