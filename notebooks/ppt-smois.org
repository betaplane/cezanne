...
###
#+RESULTS:
[[file:./obipy-resources/ppt-smois/ransac_2016.png]]
...
# ###
#+RESULTS:
[[file:./obipy-resources/ppt-smois/SMAP_pp_mm_comparison.png]]

#+begin_src ipython :results raw :session :savefig "SMAP_sta_grid.png"
  fig, ax = plt.subplots(subplot_kw={'projection': crs.PlateCarree()}, figsize=(10, 10))
  colors = plt.get_cmap('tab20').colors
  dx, dy = np.diff(smap.col).mean(), abs(np.diff(smap.row).mean())
  for k, (p, s) in enumerate(ij.items()):
      ax.scatter(*sta.loc[s, ['lon', 'lat']].as_matrix().astype(float).T, transform=crs.PlateCarree(), color=colors[k%20])
      r = smap.row.values[p[0]]
      c = smap.col.values[p[1]]
      ax.add_patch(patches.Rectangle((c-dx/2, r-dy/2), dx, dy, facecolor=colors[k%20], alpha=0.5))
...
#+RESULTS:
[[file:./obipy-resources/ppt-smois/SMAP_sta_grid.png]]
...
#+begin_src ipython :results raw :session :savefig "reg_map.png"
  fig, axs = plt.subplots(1, 2, subplot_kw={'projection': crs.PlateCarree()}, figsize=(10, 7))
  ...
  err_norm = (lambda x: cm.colors.Normalize(-x, x))(max(abs((yh - y)[rs.inlier_mask_])))
  orig_sm, error_sm = sm(y, 'cividis'), sm(yh - y, 'PiYG_r', err_norm)
  for k, (p, s) in enumerate(ij.items()):
      r = smap.row.values[p[0]]
      c = smap.col.values[p[1]]
      axs[0].add_patch(patches.Rectangle((c-dx/2, r-dy/2), dx, dy, 
                                         facecolor=orig_sm[1][k], linewidth=0))
      patchd = {'linewidth': 0, 'zorder': 1}
      if not rs.inlier_mask_[k]:
          patchd = {'edgecolor': 'orange', 'linewidth': 2, 'zorder': 10}
      axs[1].add_patch(patches.Rectangle((c-dx/2, r-dy/2), dx, dy, facecolor=error_sm[1][k], **patchd))
  ...
  plt.colorbar(orig_sm[0], ax=axs[0])
  plt.colorbar(error_sm[0], ax=axs[1])
#+end_src
...
###
#+RESULTS:
[[file:./obipy-resources/ppt-smois/9I8Swi.png]]
