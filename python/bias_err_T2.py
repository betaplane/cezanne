...
z = {'d01':'d01', 'd02':'d02', 'd03_0_00':'d03_op', 'd03_0_12':'d03_op', 'd03_orl':'d03_orl', 'fnl':'fnl'}
...
bottom row: bias, adjusted for 6.5K/km lapse rate
	"""
	fig = plt.figure(figsize=(8,8), subplotpars=SubplotParams(left=0.08,right=0.86,wspace=0.06,hspace=0.04))
	for k in range(3):
		for j,x in enumerate(Tm.minor_axis):
			...
			if k==0:
				ax.set_title(x)
				D = pd.concat((sta.loc[b.index,('lon','lat')],dz),axis=1).dropna()
				m = 1750
			elif k==1:
				D = pd.concat((sta.loc[:,('lon','lat')],b),axis=1).dropna()
				m = 13
			else:
				D = pd.concat((sta.loc[:,('lon','lat')],b-0.0065*dz),axis=1).dropna()
			sm = cm.ScalarMappable(norm=colors.Normalize(vmin=-m, vmax=m))
			...
			if j==3: 
				bb = ax.get_position()
				plt.colorbar(sm, cax=fig.add_axes([bb.x1+0.02,bb.y0,0.02,bb.y1-bb.y0]))

def bias_maps2():
	fig = plt.figure(figsize=(8,8), subplotpars=SubplotParams(left=0.08,right=0.86,wspace=0.06,hspace=0.06))
	cols = 4
	for j,x in enumerate(['d02', 'd03_orl', 'd03_0_00', 'd03_0_12']):	
		...
		for k in range(3):	
			...
			ma.drawmeridians(range(-72,-69,1),labels=[0,0,0,max(k-1,0)])
	
			if j==cols-1: 
				bb = ax.get_position()
				plt.colorbar(p, cax=fig.add_axes([bb.x1+0.02,bb.y0,0.02,bb.y1-bb.y0]))
	fig.show()

fig = plt.figure(figsize=(10,8), subplotpars=SubplotParams(left=0.08,right=0.86,wspace=0.06,hspace=0.06))
cols = 5
...
bottom row: rms, bias removed
	"""
	fig = plt.figure(figsize=(10,8), subplotpars=SubplotParams(left=0.08,right=0.86,wspace=0.06,hspace=0.04))
	for k in range(3):
		for j,x in enumerate(Tm.minor_axis):
			ax = plt.subplot(3,4,k*4+j+1)
			...
			if k==0:
				...
				b = B.std().dropna()
				D = pd.concat((sta.loc[:,('lon','lat')],b),axis=1).dropna()
			sm = cm.ScalarMappable(norm=colors.Normalize(vmin=b.min(), vmax=b.max()))
			sm.set_array(D.iloc[:,-1])
			sm.set_cmap('gnuplot')
			for i,a in D.iterrows():
				map.plot(a['lon'],a['lat'],'o',color=sm.to_rgba(a.iloc[-1]),latlon=True)
...
def mae_maps():
	...
	cmax = cmin.copy()
	p = [[],[],[]]
	fig,axs = plt.subplots(3,ncols,figsize=(10,8), subplotpars=SubplotParams(left=0.08,right=0.86,wspace=0.06,hspace=0.04))
	for j,x in enumerate(cols):
		...
		for k in range(3):
			...
			ma.drawmeridians(range(-72,-69,1),labels=[0,0,0,max(k-1,0)])
			if j==ncols-1: 
				for i in range(ncols): p[k][i].set_clim((min(cmin[k,:]),max(cmax[k,:])))
				bb = axs[k,j].get_position()
				plt.colorbar(p[k][j], cax=fig.add_axes([bb.x1+0.02,bb.y0,0.02,bb.y1-bb.y0]))
	fig.show()


def rms_seasonal():
	fig = plt.figure(figsize=(9,8), subplotpars=SubplotParams(left=0.08,right=0.86,wspace=0.06,hspace=0.04))
	for k in range(4):
		for j,x in enumerate(Tm.minor_axis):
	...
	# 		b = ((B-B.mean())**2).groupby(lambda idx: int((idx.month%12)/3))
			...
			D = pd.concat((sta.loc[c.index,('lon','lat')],c),axis=1).dropna()

			sm = cm.ScalarMappable(norm=colors.Normalize(vmin=c.min(), vmax=c.max()))
			sm.set_array(D.iloc[:,-1])
			sm.set_cmap('gnuplot')
			for i,a in D.iterrows():
				map.plot(a['lon'],a['lat'],'o',color=sm.to_rgba(a.iloc[-1]),latlon=True)
			map.drawcoastlines()
			map.drawparallels(range(-32,-28,1),labels=[max(1-j,0),0,0,0])
			map.drawmeridians(range(-72,-69,1),labels=[0,0,0,max(k-2,0)])
			plt.colorbar(sm)
			
def cycle_bias():		
	fig = plt.figure(figsize=(12,8), subplotpars=SubplotParams(left=0.06,right=0.96,wspace=0.06,hspace=0.04))
	for k in range(3):
		for j in range(5):
...
def cycles():
	fig = plt.figure(figsize=(10,8), subplotpars=SubplotParams(left=.1,right=.96,bottom=.06,top=.92,wspace=.1,hspace=.12))
	for k in range(4):
		for j in range(5):
...
def period():
	fig = plt.figure(figsize=(12,6), subplotpars=SubplotParams(left=.04,right=.96,bottom=.06,top=.92,wspace=.1,hspace=.12))
	for k in range(2):
		for j in range(5):
...
def period_rescaled():
	fig = plt.figure(figsize=(12,6), subplotpars=SubplotParams(left=.04,right=.9,bottom=.06,top=.92,wspace=.1,hspace=.12))
	for k in range(2):
		for j in range(5):
			ax = plt.subplot(2,5,k*5+j+1)
			if j==0: B = T
...
def cycle_maps():
	...
	cmax = cmin.copy()
	fig,axs = plt.subplots(2, ncols, figsize=(12,6), subplotpars=SubplotParams(left=.04,right=.9,bottom=.06,top=.92,wspace=.1,hspace=.12))
	plt.set_cmap('gnuplot')
	p = [[],[]]
	for j,x in enumerate(cols):	
		if x=='obs': B = T
