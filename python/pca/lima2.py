...
for m in np.arange(0, 1, .1):
    ...
    for k in range(1, 4):
        ...
        try:
            p1 = core.probPCA(d.x.shape, logdir='lima_logs', dims=k)
            p1.run(d.x1, n_iter=20000)
            p1.critique(d, file_name='lima2.h5', table_name='edward/results')
        except:
            pass
        else:
            P.append(p1)
        ...
        try:
            p2 = core.vbPCA(d.x1, n_iter=2000, rotate=True, K=k)
            p2.critique(d, file_name='lima2.h5', table_name='bayespy/results', rotate=False)
        except:
            pass
        else:
